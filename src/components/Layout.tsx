...
export default function Layout() {
  ...
  const nav = ({ isActive }: { isActive: boolean }) =>
    `flex-1 text-center sm:flex-none px-2 sm:px-3 py-1.5 sm:py-2 rounded-md text-sm font-medium ${isActive ? 'bg-slate-800 text-white' : 'text-slate-300 hover:text-white'}`;
  return (
    <div className="flex h-full flex-col">
      ...
      <header className="flex flex-wrap items-center gap-x-4 gap-y-1 bg-slate-900 px-3 py-2 sm:px-4">
        ...
        <a
          ...
          aria-label="GitHub"
          className="order-2 ml-auto p-1 text-slate-400 hover:text-white sm:order-5 sm:ml-0"
        >
        ...
        <button
          ...
          className="order-2 rounded-md border border-slate-600 px-2 py-1 text-sm text-slate-200 hover:bg-slate-700 sm:order-5 sm:ml-0"
